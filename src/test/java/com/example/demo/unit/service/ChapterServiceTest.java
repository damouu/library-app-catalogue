...
//        List<Chapter> chapters = Stream.generate(() -> Instancio.of(Chapter.class).set(field(Chapter::getUuid), uuids.get(0)).create()).limit(1).toList();
//        Mockito.when(chapterRepository.findByUuidAndDeletedAtIsNull(uuids)).thenReturn(chapters);
//        HashMap<String, List<Chapter>> responseEntity = chapterService.getChapters(uuids);
//        Mockito.verify(chapterRepository, Mockito.times(1)).findByUuidInAndDeletedAtIsNull(uuids);
...
//        Assertions.assertSame(responseEntity.get("chapters").getFirst().getUuid(), uuids.getFirst());
//    }
//
//    @Test
