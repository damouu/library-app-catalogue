...
public class ChapterMapper {
    ...
    public ChapterCreatedEventData toEventData(Chapter chapter, Integer initial_copies_count) {
        ...
        return new ChapterCreatedEventData(
                chapter.getUuid(),
                chapter.getSeries().getUuid(),
                chapter.getTitle(),
                chapter.getSecondTitle(),
                chapter.getTotalPages(),
                chapter.getChapterNumber(),
                chapter.getSummary(),
                chapter.getCoverArtworkUrl(),
                String.valueOf(chapter.getPublicationDate()),
                initial_copies_count
        );
    }
    ...
    public Chapter toEntity(CreateChapterRequest request, Series series) {
        ...
        return Chapter.builder()
                ...
                .title(request.title())
                .secondTitle(request.second_title())
                .chapterNumber(request.chapter_number())
                .totalPages(request.total_pages())
                .coverArtworkUrl(request.cover_artwork_url())
    ...
    public ChapterSummaryDTO toSummaryDto(Chapter chapter) {
        ...
        SeriesSummaryDTO seriesDTO = new SeriesSummaryDTO(
                chapter.getSeries().getUuid(),
                chapter.getSeries().getTitle(),
                chapter.getSeries().getGenre(),
                chapter.getSeries().getCoverArtworkUrl(),
                chapter.getSeries().getAuthor(),
                chapter.getSeries().getIllustrator(),
                chapter.getSeries().getPublisher(),
                chapter.getSeries().getFirstPrintPublicationDate(),
                chapter.getSeries().getLastPrintPublicationDate());
